...
function varargout = min (varargin)
  ...
  if nargin == 1 || nargin == 2
    args = cellfun(@gpuArray, varargin, "UniformOutput", false);
  end
  [varargout{:}] = __ocl_mat_min__ (args{:});
endfunction

%!assert (gather(min(gpuArray([3 6]), [5 4])), [3,4])
%!assert (gather(min(gpuArray([3 2 4]))), 2)
